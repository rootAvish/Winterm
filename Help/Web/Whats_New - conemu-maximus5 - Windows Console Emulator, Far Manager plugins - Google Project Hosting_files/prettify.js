...
d;++t){w=a[t];if(w.global||w.multiline)throw Error(""+w);s.push("(?:"+h(w)+")")}return RegExp(s.join("|"),f?"gi":"g")}function O(a){var b=0;return function(m){for(var n=null,h=0,r=0,v=m.length;r<v;++r){var f=m.charAt(r);switch(f){case "\t":n||(n=[]);n.push(m.substring(h,r));h=a-b%a;for(b+=h;0<=h;h-=16)n.push("                ".substring(0,h));h=r+1;break;case "\n":b=0;break;default:++b}}if(!n)return m;n.push(m.substring(h));return n.join("")}}function D(a,b,m,n){b&&(a={source:b,basePos:a},m(a),n.push.apply(n,
a.decorations))}function z(a,b){var m={},n;(function(){for(var h=a.concat(b),f=[],r={},d=0,w=h.length;d<w;++d){var s=h[d],u=s[3];if(u)for(var l=u.length;0<=--l;)m[u.charAt(l)]=s;s=s[1];u=""+s;r.hasOwnProperty(u)||(f.push(s),r[u]=null)}f.push(/[\0-\uffff]/);n=I(f)})();var h=b.length,r=function(a){for(var f=a.source,t=a.basePos,d=[t,"pln"],w=0,f=f.match(n)||[],s={},u=0,l=f.length;u<l;++u){var c=f[u],q=s[c],e=void 0,p;if("string"===typeof q)p=!1;else{var k=m[c.charAt(0)];if(k)e=c.match(k[1]),q=k[0];
else{for(p=0;p<h;++p)if(k=b[p],e=c.match(k[1])){q=k[0];break}e||(q="pln")}!(p=5<=q.length&&"lang-"===q.substring(0,5))||e&&"string"===typeof e[1]||(p=!1,q="src");p||(s[c]=q)}k=w;w+=c.length;if(p){p=e[1];var A=c.indexOf(p),g=A+p.length;e[2]&&(g=c.length-e[2].length,A=g-p.length);q=q.substring(5);D(t+k,c.substring(0,A),r,d);D(t+k+A,p,K(q,p),d);D(t+k+g,c.substring(g),r,d)}else d.push(t+k,q)}a.decorations=d};return r}function x(a){var b=[],m=[];a.tripleQuotedStrings?b.push(["str",/^(?:\'\'\'(?:[^\'\\]|\\[\s\S]|\'{1,2}(?=[^\']))*(?:\'\'\'|$)|\"\"\"(?:[^\"\\]|\\[\s\S]|\"{1,2}(?=[^\"]))*(?:\"\"\"|$)|\'(?:[^\\\']|\\[\s\S])*(?:\'|$)|\"(?:[^\\\"]|\\[\s\S])*(?:\"|$))/,
...
null,"#"]),m.push(["str",/^<(?:(?:(?:\.\.\/)*|\/?)(?:[\w-]+(?:\/[\w-]+)+)?[\w-]+\.h|[a-z]\w*)>/,null])):b.push(["com",/^#[^\r\n]*/,null,"#"]));a.cStyleComments&&(m.push(["com",/^\/\/[^\r\n]*/,null]),m.push(["com",/^\/\*[\s\S]*?(?:\*\/|$)/,null]));a.regexLiterals&&m.push(["lang-regex",RegExp("^"+P+"(/(?=[^/*])(?:[^/\\x5B\\x5C]|\\x5C[\\s\\S]|\\x5B(?:[^\\x5C\\x5D]|\\x5C[\\s\\S])*(?:\\x5D|$))+/)")]);a=a.keywords.replace(/^\s+|\s+$/g,"");a.length&&m.push(["kwd",RegExp("^(?:"+a.replace(/\s+/g,"|")+")\\b"),
null]);b.push(["pln",/^\s+/,null," \r\n\t\u00a0"]);m.push(["lit",/^@[a-z_$][a-z_$@0-9]*/i,null],["typ",/^@?[A-Z]+[a-z][A-Za-z_$@0-9]*/,null],["pln",/^[a-z_$][a-z_$@0-9]*/i,null],["lit",/^(?:0x[a-f0-9]+|(?:\d(?:_\d+)*\d*(?:\.\d*)?|\.\d\+)(?:e[+\-]?\d+)?)[a-z]*/i,null,"0123456789"],["pun",/^.[^\s\w\.$@\'\"\`\/\#]*/,null]);return z(b,m)}function Q(a){function b(a){if(a>t){d&&d!==w&&(f.push("</span>"),d=null);!d&&w&&(d=w,f.push('<span class="',d,'">'));var b=y(l(m.substring(t,a))).replace(k?q:c,"$1&#160;");
k=p.test(b);f.push(b.replace(e,x));t=a}}var m=a.source,n=a.extractedTags,h=a.decorations,r=a.numberLines,v=a.sourceNode,f=[],t=0,d=null,w=null,s=0,u=0,l=O(window.PR_TAB_WIDTH),c=/([\r\n ]) /g,q=/(^| ) /gm,e=/\r\n?|\n/g,p=/[ \r\n]$/,k=!0,g=window._pr_isIE6(),v=g?v&&"PRE"===v.tagName?6===g?"&#160;\r\n":7===g?"&#160;<br />\r":8===g?"&#160;<br />":"&#160;\r":"&#160;<br />":"<br />",x;if(r){for(var J=[],g=0;10>g;++g)J[g]=v+'</li><li class="L'+g+'">';var z="number"===typeof r?r-1:0;f.push('<ol class="linenums"><li class="L',
